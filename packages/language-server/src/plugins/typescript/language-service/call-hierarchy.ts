...
import { findNodeAtSpan, gatherDescendants } from '../features/utils';
import {
    isGeneratedSvelteComponentName,
    isSvelteFilePath,
    offsetOfGeneratedComponentExport
} from '../utils';
import { LSPluginInfo, SvelteSnapshot } from './interfaces';
...
export function decorateCallHierarchy({
    ...
    const prepareCallHierarchy = lang.prepareCallHierarchy;
    const provideCallHierarchyIncomingCalls = lang.provideCallHierarchyIncomingCalls;
    const provideCallHierarchyOutgoingCalls = lang.provideCallHierarchyOutgoingCalls;

    lang.prepareCallHierarchy = (fileName: string, position: number) => {
        ...
        const items = prepareCallHierarchy(fileName, position);
        ...
        return itemsArray
            .map((item) => convertSvelteCallHierarchyItem(item, program))
            .filter(isNotNullOrUndefined);
    };

    lang.provideCallHierarchyIncomingCalls = (fileName: string, position: number) => {
        const program = lang.getProgram();
        ...
        const componentExportOffset =
            isComponentModulePosition(fileName, position) && snapshot
                ? offsetOfGeneratedComponentExport(snapshot.getGeneratedText())
                : -1;
    ...
    lang.provideCallHierarchyOutgoingCalls = (fileName: string, position: number) => {
        const program = lang.getProgram();
        // probably won't happen
        if (!program) {
    ...
    function componentRefToIncomingCall(
        ...
        const startTags = ref.references
            .map((ref) => {
                const node = findNodeAtSpan(sourceFile, ref.textSpan, isComponentStartTag);

                if (!node) {
                    return null;
                }

                const textSpan = snapshot.getOriginalTextSpan(ref.textSpan);
                return textSpan;
            })
            .filter(isNotNullOrUndefined);
