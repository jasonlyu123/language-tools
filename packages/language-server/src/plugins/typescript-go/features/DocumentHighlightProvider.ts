...
import { Document, inStyleOrScript, isInTag, mapRangeToOriginal } from '../../../lib/documents';
import { isSamePosition } from '../../../utils';
import { DocumentHighlightProvider } from '../../interfaces';
// @ts-ignore
import { TemplateNode } from 'svelte/types/compiler/interfaces';
import { walkSvelteAst } from '../../typescript/svelte-ast-utils';
...
export class TsGoDocumentHighlightProvider implements DocumentHighlightProvider {
    ...
    async findDocumentHighlight(
        ...
        if (generatedPos.line < 0) {
            return null;
        }
        const highlights = await this.tsApiService.sendRequest(DocumentHighlightRequest.type, {
        ...
        const result = highlights
            .filter(this.notInGeneratedCode(tsDoc))
            .map((highlight) =>
                DocumentHighlight.create(mapRangeToOriginal(tsDoc, highlight.range), highlight.kind)
            )
    ...
    private notInGeneratedCode(tsDoc: SvelteDocumentSnapshot) {
        return (ref: DocumentHighlight) => {
            return !isRangeInGeneratedCode(tsDoc, ref.range);
        };
    }
}

export async function getSvelteDocumentHighlight(
    document: Document,
    ...
    const candidate = findCandidateSvelteTag(tsDoc, offset);
    ...
    if (candidate.type.endsWith('Tag')) {
        return getTagHighlight(offset, document, candidate);
    }

    if (candidate.type.endsWith('Block')) {
        return getBlockHighlight(offset, document, candidate);
    }

    return null;
}

function findCandidateSvelteTag(tsDoc: SvelteDocumentSnapshot, offset: number) {
